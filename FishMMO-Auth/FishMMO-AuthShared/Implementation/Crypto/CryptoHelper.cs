...
namespace FishMMO.Auth.Implementation
{
	...
	public static class CryptoHelper
	{
		...
		/// </summary>
		/// <summary>Token format v4: added realIp field for World/Scene server DoS protection.</summary>
		...
		/// Layout (v4): [1B version][1B tokenType][2B nameLen BE][name UTF-8][1B accessLevel][8B serverId BE][8B signingKeyId BE][8B ticks BE][1B ipLen][realIp UTF-8][16B nonce][32B HMAC].
		...
		public static byte[] BuildAuthToken(string accountName, long loginServerId, long signingKeyId, DateTime expiresUtc, byte[] hmacKey, AccessLevel accessLevel, string? realIp = null)
		{
			...
			// Layout (v4): [1B version][1B tokenType][2B nameLen][name][1B accessLevel][8B serverId][8B signingKeyId][8B ticks][1B ipLen][realIp][16B nonce]
			// v4: realIp field
			byte[] ipBytes = realIp != null ? Encoding.UTF8.GetBytes(realIp) : Array.Empty<byte>();
			if (ipBytes.Length > 255) throw new ArgumentException("realIp too long.", nameof(realIp));
			int payloadLength = 1 + 1 + 2 + nameBytes.Length + 1 + 8 + 8 + 8 + 1 + ipBytes.Length + 16;
			...
			// Real IP (v4: 1B length + bytes)
			payload[offset++] = (byte)ipBytes.Length;
			if (ipBytes.Length > 0)
			{
				Buffer.BlockCopy(ipBytes, 0, payload, offset, ipBytes.Length);
		...
		/// </summary>
		public const int MinSignedTokenLength = 1 + 1 + 2 + 1 + 1 + 8 + 8 + 8 + 1 + 0 + 16 + HmacTagLength; // v4: +1B ipLen
		...
		public static class TwoFactor
		{
			...
			/// </summary>
			/// <remarks>
			/// <para><b>IL2CPP / AOT limitation:</b> On Unity IL2CPP and other AOT compilation targets,
			/// the runtime strips reflection metadata from non-public fields by default. This causes
			/// <see cref="System.Reflection.BindingFlags.NonPublic"/> field lookups to silently return
			/// no results, making this zeroization a no-op. The OtpNet <c>Totp</c> object graph
			...
			private static void TryZeroizeTotpInternals(object totp)
			{
				if (totp == null) return;
#if ENABLE_IL2CPP
				System.Diagnostics.Debug.WriteLine("[CryptoHelper.TryZeroizeTotpInternals] IL2CPP build: reflection-based zeroization is a no-op. Rely on caller-side ZeroMemory only.");
				return;
#endif
				try
				{
