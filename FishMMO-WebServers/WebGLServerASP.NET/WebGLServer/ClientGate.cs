	...
	internal static class ClientGate
	{
		...
		public static IApplicationBuilder UseClientGate(this IApplicationBuilder app, IHostEnvironment environment, params string[] bypassPaths)
		{
			...
			return app.Use(async (ctx, next) =>
			{
				...
				// Collapse repeated slashes and reject traversal segments so the signed canonical string cannot be desynchronised
				// from the routing target via path-equivalence tricks.
		...
		/// <summary>
		/// Normalizes the request path so client and server
