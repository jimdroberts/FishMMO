...
export function historyBody(ui, history, excludeTicketId = null) {
	...
	const priorReports = against.totalCount ?? 0;
	...
	return `
		<div class="stack">
			<div class="grid grid-3">
				...
				${ui.stat({ label: 'Tickets filed', value: ui.num(filed.totalCount) })}
				${ui.stat({ label: 'Staff actions', value: ui.num(history.moderation?.length ?? 0), note: lockedCharacters ? `${lockedCharacters} character(s) locked` : '' })}
			</div>
