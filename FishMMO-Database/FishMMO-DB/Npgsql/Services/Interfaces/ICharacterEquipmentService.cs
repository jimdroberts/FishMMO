...
namespace FishMMO.Database.Npgsql.Services.Interfaces
{
		...
		IFetchCollectionByKeyAction<long, CharacterEquipmentData>
	{
		...
		/// <summary>
		/// Replaces the character's entire equipment with an authoritative snapshot of the server's
		...
		/// </para>
		/// </remarks>
		/// <param name="characterId">The character whose equipment is being replaced.</param>
		/// <param name="items">Every occupied slot. An empty or null collection empties the container.</param>
		/// <param name="cancellationToken">Token to cancel the operation.</param>
		/// <returns>A <see cref="DatabaseResult"/> indicating success or failure.</returns>
		Task<DatabaseResult> SaveSnapshotAsync(long characterId, IEnumerable<CharacterEquipmentData> items, CancellationToken cancellationToken = default);
	}
}