...
namespace FishMMO.Database.Npgsql.Services
{
	...
	public sealed class CharacterPetAttributeService : BaseService<CharacterPetAttributeEntity>, ICharacterPetAttributeService
	{
		...
		/// </summary>
		private static readonly Func<NpgsqlDbContext, long, IAsyncEnumerable<CharacterPetAttributeEntity>> getAttributesQuery =
		...
		/// <inheritdoc/>
		public Task<DatabaseResult<BulkWriteResult>> PersistAsync(IEnumerable<CharacterPetAttributeData> attributes, CancellationToken cancellationToken = default)
			=> PersistBatchAsync(attributes, null, cancellationToken);

		/// <inheritdoc/>
		public Task<DatabaseResult<BulkWriteResult>> PersistOwnedAsync(IEnumerable<CharacterPetAttributeData> attributes, IReadOnlyCollection<CharacterSessionLeaseData> claims, CancellationToken cancellationToken = default)
		{
			...
			return invalid != null
				...
				: PersistBatchAsync(attributes, claims, cancellationToken);
		}

		/// <summary>
		/// The batch write behind <see cref="PersistAsync(IEnumerable{CharacterPetAttributeData}, CancellationToken)"/> and
		/// <see cref="PersistOwnedAsync"/>.
		/// </summary>
		/// <param name="attributes">Rows to write.</param>
		/// <param name="claims">The writer's claims, or null for the ungated write. See <see cref="CharacterWriteGate"/>.</param>
		/// <param name="cancellationToken">Cancellation token.</param>
		private async Task<DatabaseResult<BulkWriteResult>> PersistBatchAsync(IEnumerable<CharacterPetAttributeData> attributes, IReadOnlyCollection<CharacterSessionLeaseData>? claims, CancellationToken cancellationToken)
		{
			...
			list = BulkBatch.KeepNewest(list, attribute => (attribute.CharacterID, attribute.TemplateID), attribute => attribute.Version);
			...
			return await ExecuteTransactionAsync<BulkWriteResult>(async dbContext =>
			{
				...
				var activeAttributes = list.Where(a => admission.Admits(a.CharacterID)).ToList();
				...
				return new BulkWriteResult(suppliedRows, activeAttributes.Count, appliedRows, unownedRows);
			}, saveChanges: false, cancellationToken: cancellationToken).ConfigureAwait(false);
		}
