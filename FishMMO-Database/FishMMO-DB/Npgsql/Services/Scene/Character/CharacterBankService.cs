...
namespace FishMMO.Database.Npgsql.Services
{
	...
	public sealed class CharacterBankService : BaseService<CharacterBankEntity>, ICharacterBankService
	{
		...
		public async Task<DatabaseResult<long>> PersistAsync(CharacterBankData item, CancellationToken cancellationToken = default)
		{
			...
			var result = await ExecuteTransactionAsync(async dbContext =>
			{
				...
				var id = await ExecuteScalarLongAsync(
						...
						new object[] { item.CharacterID, item.Slot, item.Version, item.TemplateID, item.Seed, (long)item.Amount, now },
		...
		/// <inheritdoc/>
		public async Task<DatabaseResult<BulkWriteResult>> PersistAsync(IEnumerable<CharacterBankData> items, CancellationToken cancellationToken = default)
		{
