...
namespace FishNet.Serializing
{
	...
	public partial class Reader
	{
		...
		/// <summary>
		/// Decodes a ZigZag-encoded unsigned 64-bit integer back to its original signed value.
		///
		/// Mapping:   0 → 0,  1 → -1,  2 → 1,  3 → -2,  4 → 2,  …
		/// </summary>
		/// <param name="value">The ZigZag-encoded unsigned 64-bit integer to decode.</param>
		/// <returns>The original signed 64-bit integer.</returns>
		...
		/// <summary>
		/// Reads a variable-length packed whole number and applies ZigZag decoding to
		/// recover the original signed value.
		/// </summary>
		/// <returns>The decoded signed 64-bit integer.</returns>
		public long ReadSignedPackedWhole() => ZigZagDecode(ReadUnsignedPackedWhole());
