...
namespace FishMMO.TestHarness
{
	...
	public sealed class CombatSimBootstrap : MonoBehaviour
	{
		...
		private IEnumerator Start()
		{
			...
			// 3b. NPC brains, the same host the scene server's AISystem runs.
		...
		/// <summary>Flat arena floor plus a runtime-built NavMesh (the brain warps its agent when
		/// it is prepared and stalls without one).</summary>
		...
		private void SpawnFighter(int team, int slot)
		{
			...
			GameObject clone = server.Spawn(Manifest.NpcPrefab, position, facing, gameObject.scene,
				...
				afterActivate: go =>
				{
					/* The archetype is a shared asset — mutating it would edit the shipped brain — so
					 * each fighter gets a per-instance CLONE to tune, handed to the host as this
					 * spawn's override, the way a spawner hands over its own. */
					AIArchetypeTemplate simBrain = SimBrains.CloneArchetype(brainHost, Manifest.NpcPrefab, TuneForStandoff);
		...
		/// <summary>
		/// Holds a ranged standoff instead of the shipped melee spacing.
		/// </summary>
		/// <remarks>
		/// The shipped attacking state has PreferredDistance 0 — close to melee. Two such teams
		/// collapse into a touching scrum, and aiming at a target you are standing inside produces a
		/// near-vertical fire direction (projectiles straight up). A cloned state holds a ranged
		/// standoff instead, so the red and blue lines trade fire visibly.
		/// </remarks>
		private static void TuneForStandoff(AIArchetypeTemplate simBrain)
		{
			...
			// Global cast pacing on top of per-ability cooldowns: without it a spellbook this deep
			// lets the AI legally chain instants back-to-back. The jitter is also what
			// desynchronizes the two teams' rhythms.
