...
namespace FishMMO.UnitTests
{
	...
	public class AbilityPersistenceDirtyTests
	{
		...
		[Test]
		public void ChangingTheEventSet_AdvancesTheVersion()
		{
			/* The mark says an ability has changed; the version says *which* change the database
			 * has. MarkPersisted can only tell a change made during a save from one already stored
			 * by comparing versions, so a mutation that left the version alone would be
			 * indistinguishable from no mutation at all -- see the in-flight test below, which is
			 * the case that would silently lose a change without this. */
			Ability ability = NewAbility();
			long before = ability.Version;

			AbilityOnHitEvent abilityEvent = ScriptableObject.CreateInstance<AbilityOnHitEvent>();
			try
			{
				abilityEvent.name = "PersistenceDirtyVersionEvent";
				abilityEvent.AddToCache(abilityEvent.name);

				ability.AddEvent(abilityEvent);
				long afterAdd = ability.Version;

				Assert.Greater(afterAdd, before,
					"Learning an event has to move the version, or the save cannot tell it happened.");

				ability.RemoveAbilityEvent(abilityEvent.ID);

				Assert.Greater(ability.Version, afterAdd,
					"Forgetting one has to move it as well.");
			}
			finally
			{
				abilityEvent.RemoveFromCache();
		...
		public void MarkPersisted_LeavesItDirtyWhenItChangedWhileTheSaveWasInFlight()
		{
			 ...
			 * until the ability happened to change again -- which, for an ability, may be never.
			 *
			 * Driven through the real mutation path rather than by moving Version by hand: it is
			 * exactly the question of whether that path moves the version that decides whether the
			 * guard below holds or is a no-op. */
			Ability ability = NewAbility();
			ability.Version++;
			...
			try
			{
				abilityEvent.name = "PersistenceDirtyInFlightEvent";
				abilityEvent.AddToCache(abilityEvent.name);

				// The save is in flight; the player learns an event.
				ability.AddEvent(abilityEvent);
