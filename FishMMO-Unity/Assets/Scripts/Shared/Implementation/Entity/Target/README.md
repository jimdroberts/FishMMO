...
1. Ensure the character has a `TargetController` component (added automatically via `[RequireComponent]` on both `PlayerCharacter` and `NPC`).
...
- **NPC** — `TargetController` is also a `[RequireComponent]` on `NPC`. An NPC never runs the mouse path (it is owner-only and compiled out of dedicated servers); the server-side brain (`AIController`, in the server assembly and attached only when the server spawns the NPC) solves an aim that `AbilityController` reads through `INPCBrain.AimRotation` and replicates as the NPC's aim, and `UpdateTarget()` traces from that aim exactly as it does for a player. Without the component the NPC's casts complete and spawn nothing — see issue #232 and `FishMMO Dashboard → Core → Validate → Audit NPC Prefabs`. An **immortal** NPC (authored on `CharacterDamageController`, or a corpse) has no reason to target anything: `UpdateTarget()` short-circuits to a miss for it before any trace, and the repair/audit tooling leaves its mask alone.
