...
namespace FishMMO.Shared
{
	...
	public class Trigger : ScriptableObject
	{
		...
		public void Execute(EventData eventData)
		{
			...
			{
				Debug.LogWarning($"Trigger '{name}' attempted to execute without a valid Initiator.");
				return;
			}
			...
			foreach (var condition in Conditions)
			{
				if (condition != null)
				{
					...
					{
						Debug.Log($"Trigger '{name}' conditions not met for {eventData.Initiator?.Name}. Event: {eventData.GetType().Name}.");
						return;
					}
				}
			}
			...
			Debug.Log($"Trigger '{name}' conditions met for {eventData.Initiator?.Name}. Executing actions for Event: {eventData.GetType().Name}...");
