...
namespace FishMMO.Shared
{
    ...
    public class FactionCondition : BaseCondition<IPlayerCharacter> // Assuming BaseConditionSO from previous discussion
    {
        ...
        public int MinimumReputationValue; // Only used for more granular checks beyond AllianceLevel
        ...
        public override bool Evaluate(IPlayerCharacter playerCharacter)
        {
            ...
            if (!playerCharacter.TryGet(out IFactionController playerFactionController))
            {
                Debug.LogWarning($"FactionCondition: Player character {playerCharacter.CharacterName} does not have an IFactionController.");
                return false;
            }
