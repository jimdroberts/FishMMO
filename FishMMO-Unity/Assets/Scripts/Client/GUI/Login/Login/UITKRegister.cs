...
namespace FishMMO.Client
{
	...
	public class UITKRegister : UITKControl
	{
		...
		/// <summary>Verify-by-Discord toggle.</summary>
		private const string VERIFY_DISCORD_NAME = "register-verify-discord";
		/// <summary>Discord username field; needed only to verify by Discord.</summary>
		private const string DISCORD_NAME = "register-discord";
		/// <summary>Where to join the game's Discord server; hidden unless an invite link is set.</summary>
		private const string DISCORD_HINT_NAME = "register-discord-hint";
		...
		private Toggle verifyByDiscord;
		private TextField discordUsername;
		private Label discordHint;

		/// <summary>
		/// The invite link to the game's Discord server, shown beside the Discord username field.
		/// </summary>
		/// <remarks>
		/// A player who verifies by Discord must be in that server: the bot can only message members of a
		/// server it shares with them, and only finds a username among its own server's members. Empty hides
		/// the hint. Only an https link is shown or opened.
		/// </remarks>
		[SerializeField]
		[Tooltip("https invite link to the Discord server the FishMMO Discord bot runs in. Empty hides the hint.")]
		...
		public override void OnStarting()
		{
			...
			verifyByDiscord = Root.Q<Toggle>(VERIFY_DISCORD_NAME);
			discordUsername = Root.Q<TextField>(DISCORD_NAME);
			discordHint = Root.Q<Label>(DISCORD_HINT_NAME);
			ApplyDiscordInvite();
			...
			LoginKeys.SetTabOrder(Root, username, email, password, ageSelect, verifyByEmail, verifyBySms, verifyByDiscord, discordUsername,
			...
			switch (result)
			{
				...
				case ClientAuthenticationResult.InvalidUsernameOrPassword:
					OnRegistrationDialog(this.accountCreated
						? "That verification code was not accepted.\n\nYour account exists: sign in with it to enter a code again. After three incorrect codes a support ticket is opened for your account, and staff will contact you by email."
				...
				case ClientAuthenticationResult.AccountDetailsInvalid:
					OnRegistrationDialog("Some of the details were not accepted.\n\nCheck that the phone number starts with its country code (for example +44) and that the Discord username is written as it appears on your profile. " +
						"If you chose to verify by SMS or Discord, fill in the phone number or Discord username it needs: this server has to be able to send you a verification code.");
					break;
		...
		/// <summary>
		/// PhoneUnverified, from a server one release behind that still asked for codes one channel at a
		/// time. Any one code verifies the account now, so it means what AccountCreated means before the
		/// account exists and what AccountUnverified means after.
		/// </summary>
		...
		/// <summary>
		/// Opens the verification code input dialog. One prompt for every channel: the server tries the code
		/// against every code the account holds, and any one of them verifies it.
		/// </summary>
		private void OpenVerifyCodeDialog()
		{
			...
			const string prompt = "Enter the verification code we sent you. It may arrive by email, by text message, or as a " +
				"direct message from our Discord bot once you are in our Discord server. Any one of them verifies your account.";
			...
			if (UIManager.TryGetTK("UIDialogInputBox", out UITKDialogInputBox uiDialogInputBox))
			{
				uiDialogInputBox.OpenCode(
					...
					(code) =>
					{
						...
						{
							Client.LoginAuthenticator.SendVerifyCode(pendingVerifyUsername, code.Trim(), VerificationCodeChannel.Email);
		...
		private void ClearAllFields()
		{
			...
			foreach (TextField field in new[] { phone, country, realName, address, referral, betaCode, discordUsername })
			{
				if (field != null)
				{
					field.value = "";
		...
		/// <summary>
		/// Shows where to join the game's Discord server, when this client has an invite link. See
		/// <see cref="discordInviteUrl"/>.
		/// </summary>
		private void ApplyDiscordInvite()
		{
			if (discordHint == null)
			{
				return;
			...
			string url = InviteLink();
			discordHint.style.display = url != null ? DisplayStyle.Flex : DisplayStyle.None;
			if (url == null)
			{
				return;
			}
			discordHint.text = $"Join our Discord server first so the bot can message you: {url}";
			// A method group, so a second OnStarting over the same tree does not open the link twice.
			discordHint.RegisterCallback<ClickEvent>(OnDiscordHintClicked);
		}

		/// <summary>The configured invite link when it is an https link, otherwise null.</summary>
		private string InviteLink()
		{
			string url = discordInviteUrl == null ? null : discordInviteUrl.Trim();
			return !string.IsNullOrEmpty(url) && url.StartsWith("https://", System.StringComparison.OrdinalIgnoreCase) ? url : null;
		}

		private void OnDiscordHintClicked(ClickEvent evt)
		{
			string url = InviteLink();
			if (url != null)
			{
		...
		/// <summary>Email verification on, SMS and Discord off: the choice every player starts with.</summary>
		private void ResetVerificationChoice()
		{
			...
			verifyByDiscord?.SetValueWithoutNotify(false);
		}
		...
		private RegistrationProfile BuildProfile()
		{
			...
			if (verifyByDiscord != null && verifyByDiscord.value)
			{
				channels |= RegistrationVerificationChannels.Discord;
			}
			...
			return new RegistrationProfile
			{
				...
				DiscordUsername = NullIfBlank(discordUsername),
		...
		private void ReleaseControls(bool interactable)
		{
			...
			foreach (VisualElement control in new VisualElement[] { phone, country, realName, address, referral, betaCode, verifyByEmail, verifyBySms, verifyByDiscord, discordUsername })
			{
