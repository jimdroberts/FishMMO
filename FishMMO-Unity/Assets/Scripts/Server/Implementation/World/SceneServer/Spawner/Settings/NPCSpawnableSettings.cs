...
namespace FishMMO.Server.Implementation.World.SceneServer.Spawner
{
	...
	/// </para>
	/// <para>
	/// Everything but the archetype is applied in <see cref="OnSpawned"/>, which runs after the
	/// object leaves the pool and before <c>ServerManager.Spawn</c> — that is, before
	/// <see cref="NPC.OnStartServer"/> rolls attributes and learns abilities, and before the spawn
	/// payload is written to clients. The archetype goes to the brain host, which prepares the
	/// brain once the NPC is in its scene.
	/// </para>
	/// </remarks>
	[Serializable]
	[MovedFrom(false, sourceNamespace: "FishMMO.Shared", sourceAssembly: "FishMMO.Shared", sourceClassName: "NPCSpawnableSettings")]
	public class NPCSpawnableSettings : SpawnableSettings
	{
		...
		/// without touching the prefab. Handed to <see cref="AIBrainHost.Prepare"/> by the spawner;
		/// the brain reads every state and tuning value live from its archetype, so this one field
		/// is the whole override. Empty means the prefab's entry in the brain catalogue.
		/// </remarks>
		[Header("Behaviour")]
		...
		public override void OnSpawned(NetworkObject nob, SpawnerRuntime spawner)
		{
