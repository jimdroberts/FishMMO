...
- **One population ceiling for both servers** — `MaxClientsPerInstance` is `WorldSceneSettings.MaximumClientsPerScene` (200), the same constant the scene server clamps its admission cap to, so routing and admission cannot disagree about whether an instance has room
...
|---|---|---|
| `MaxClientsPerInstance` | `WorldSceneSettings.MaximumClientsPerScene` (`200`) | Hard cap on clients per scene instance; `GetMaxClients` clamps a scene's authored `MaxClients` to `[1, MaxClientsPerInstance]` and returns the cap when the scene is not in `worldSceneDetailsCache`. Deferred to the shared constant rather than declared here, because the scene server's own admission cap (`SceneChannelSystem.GetMaxClients`) reads the same value — a local number would let this server route players into a scene the scene server then refuses |
