...
{
	/// <summary>
	/// Server Authenticator that allows Clients to connect with basic password authentication.
	/// </summary>
	public class ServerAuthenticator : Authenticator
	{
		...
		internal void OnServerSrpVerifyBroadcastReceived(NetworkConnection conn, SrpVerifyBroadcast msg, Channel channel)
		{
						...
						if (Server.AccountManager.TryUpdateSrpState(conn, SrpState.SrpVerify, SrpState.SrpVerify, (a) =>
							{
								...
								NetworkManager.ServerManager.Broadcast(conn, srpVerify, false, Channel.Reliable);
								return true;
							}))
						{
							return;
			...
			NetworkManager.ServerManager.Broadcast(conn, authResult, false, Channel.Reliable);
		}
		...
		internal void OnServerSrpProofBroadcastReceived(NetworkConnection conn, SrpProofBroadcast msg, Channel channel)
		{
			 ...
			 * already be considered authenticated. */
			if (conn.IsAuthenticated ||
				!Server.AccountManager.TryUpdateSrpState(conn, SrpState.SrpVerify, SrpState.SrpProof, (a) =>
				{
					...
					if (a.SrpData.GetProof(clientProof, out string serverProof) &&
						Server.AccountManager.TryUpdateSrpState(conn, SrpState.SrpProof, SrpState.SrpSuccess, (a) =>
						{
							using var dbContext = NpgsqlDbContextFactory.CreateDbContext();
				...
				}))
			{
				...
				NetworkManager.ServerManager.Broadcast(conn, authResult, false, Channel.Unreliable);
