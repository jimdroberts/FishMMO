...
namespace AppHealthMonitor
{
    public class HealthMonitor
    {
        ...
        public HealthMonitor(
            ...
            CancellationToken cancellationToken) // This is now the linked token from Program.cs
        {
        ...
        public async Task StartMonitoring()
        {
            ...
            while (!this.cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{this.derivedProcessName}] Monitoring task cancelled. Exiting loop.");
        ...
        public void KillApplication() // Changed to public to be callable from Program.cs
        {
            if (this.monitoredProcess == null)
            {
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{this.derivedProcessName}] KillApplication: No active process reference to kill.");
                return;
            }

            int processId = this.monitoredProcess.Id;
            string processName = this.derivedProcessName; // Use derived name for logging consistency
            ...
            try
            {
                ...
                if (this.monitoredProcess.HasExited)
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Process ID: {processId} has already exited, no need to kill.");
                    this.monitoredProcess = null;
                    return;
                }

                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Attempting graceful shutdown for process ID: {processId}...");
                Console.ResetColor();
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    ...
                    if (this.monitoredProcess.MainWindowHandle != IntPtr.Zero)
                    {
                        ...
                        Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Sent CloseMainWindow signal (Windows).");
                        gracefulShutdownAttempted = true;
                    }
                    else
                    {
                        Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: No main window detected for graceful shutdown on Windows via CloseMainWindow. Will proceed to force kill if needed.");
                    }
                }
                else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux) || RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Relying on Process.Kill(true) for graceful termination on Unix-like OS (Linux/macOS), which sends SIGTERM to process tree.");
                    gracefulShutdownAttempted = true; 
                }
                else
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Unsupported OS for specific graceful shutdown methods. Will rely on Process.Kill(true) for all termination attempts.");
                }

                if (gracefulShutdownAttempted)
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Waiting for process ID: {processId} to exit gracefully ({this.gracefulShutdownTimeout.TotalSeconds}s timeout).");
                    if (this.monitoredProcess.WaitForExit((int)this.gracefulShutdownTimeout.TotalMilliseconds))
                    {
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Process ID: {processId} exited gracefully.");
                        Console.ResetColor();
                        this.monitoredProcess = null;
                        return;
                    }
                    ...
                    {
                        Console.ForegroundColor = ConsoleColor.Yellow;
                        Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Process ID: {processId} did not exit gracefully within {this.gracefulShutdownTimeout.TotalSeconds}s. Proceeding with force kill.");
                        Console.ResetColor();
                    }
                }
                else
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: No specific graceful shutdown method applied. Proceeding with force kill.");
                }

                // If not exited gracefully (or if graceful shutdown wasn't attempted/failed), force kill with process tree
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Force killing process ID: {processId} and its children using Process.Kill(true)...");
                Console.ResetColor();
                this.monitoredProcess.Kill(true); // Kills the process and its descendants
                
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Waiting for process ID: {processId} to confirm exit after force kill (5s timeout).");
                this.monitoredProcess.WaitForExit(5000); // Wait up to 5 seconds for the process to exit after kill

                if (this.monitoredProcess.HasExited)
                {
                    Console.ForegroundColor = ConsoleColor.Green;
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Process ID: {processId} and its children killed successfully.");
                    Console.ResetColor();
                }
                else
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Critical Warning: Process ID: {processId} did not exit even after force kill(true) command. It might be stuck!");
                    Console.ResetColor();
                }
            }
            catch (InvalidOperationException ex)
            {
                // This can happen if the process exited right before refresh or kill
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Process ID: {processId} already exited or invalid handle during kill attempt (InvalidOperationException). Error: {ex.Message}");
            }
            catch (Exception ex)
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] [Monitor-{monitorInstanceId.ToString().Substring(0, 8)}] [{processName}] KillApplication: Error during application kill for process ID: {processId}. Error: {ex.Message}");
                Console.ResetColor();
            }
            finally
            {
