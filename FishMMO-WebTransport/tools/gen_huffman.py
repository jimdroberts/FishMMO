#!/usr/bin/env python3
"""Generate RFC 7541 Appendix B Huffman tables for http3.cpp."""

# (code, bitlen) for symbols 0-255 from RFC 7541 Appendix B / nghttp2
# Verified: 'a'=00011 (5)=3, 'l'=001110 (6)=0xe, ' '=010100 (6)=0x14
HUFFMAN_SYMBOL_TABLE = [
    ...
    (0x2e, 6), (0x7a, 7), (0x7b, 7), (0x79, 7),
    (0x7c, 7), (0x7d, 7), (0x7e, 7), (0x7f, 7),
    (0x7ffe, 15), (0x7fff1, 19), (0x3ffd, 14), (0x1ffd, 13),
...
assert len(HUFFMAN_SYMBOL_TABLE) == 256, len(HUFFMAN_SYMBOL_TABLE)
assert HUFFMAN_SYMBOL_TABLE[ord('a')] == (0x3, 5)
assert HUFFMAN_SYMBOL_TABLE[ord('l')] == (0xe, 6)
assert HUFFMAN_SYMBOL_TABLE[ord(' ')] == (0x14, 6)
assert HUFFMAN_SYMBOL_TABLE[ord('0')] == (0x0, 5)

print("static const uint8_t kHuffLen[256] = {")
for i in range(0, 256, 16):
    chunk = HUFFMAN_SYMBOL_TABLE[i:i+16]
    print("   " + ", ".join(f"{ln:2d}" for _, ln in chunk) +
          ("," if i + 16 < 256 else "") +
          f"  /* {i:3d}-{i+15:3d} */")
print("};")
print()
print("static const uint32_t kHuffCode[256] = {")
for i in range(0, 256, 4):
    chunk = HUFFMAN_SYMBOL_TABLE[i:i+4]
    print("    " + ", ".join(f"0x{code:x}" for code, _ in chunk) +
          ("," if i + 4 < 256 else "") +
          f"  /* {i:3d}-{i+3:3d} */")
print("};")
