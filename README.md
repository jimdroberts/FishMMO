...
| Arena board (an `ArenaBoard` interactable) | Queues the character for a PvP arena at a chosen format (1v1, 2v2, 3v3, 4v4, 8v8 and so on, as the `ArenaTemplate` lists them; a format may be **ranked**). The board panel stays open and shows the wait; **closing it leaves the queue, and walking away from the board drops the player**. It also shows the character's **season standing** (rating once placed, wins and losses, any queue lock), a **History** tab of their recent matches and a **Leaderboard** tab of the season's top ratings. A party queues together through **Queue as Party** (leader only, every member at the board, party no larger than a team; in a ranked format the party must fill a whole team) and is kept on one team. Matches form first-come, atomically across scene servers, within a **rating band that widens with wait time** for ranked formats, and **balanced by rating** across teams when the template asks; a **vacated seat in a live match is backfilled** from the queue during the template's window. Everyone is moved into the arena instance, waits for all seats to arrive, answers a **ready check** (a decline or a silence cancels the match and locks the culprit out of the queue for a while), then a **10 second countdown** (with per-second cue hooks for audio and effects) starts the match. **Team Deathmatch** scores kills, **Capture the Flag** scores deliveries of the enemy flag to your own stand (a carrier's death **drops the flag** where they fell: their team touches it home, an enemy picks it up, or it returns by itself), **King of the Hill** scores time holding a control point; the dead respawn at their team's spawn after a **death recap** naming the killer, and the match ends on the score limit, the clock, or a walkover. A **kill feed and announcer** (first blood, killing sprees, flags, time and score warnings) run from server events, each also a template cue hook. Leaving a live match **forfeits** it and locks the deserter out of the queue; a player who **disconnects** keeps their seat for the template's grace and is refunded the forfeit if they come back in time. A results screen shows the winner, the team scores, a pedestal of the top scorers and, for ranked, the rating change, and everyone is returned to the world. Arenas **count against the one-instance-per-party rule**, and every result moves the character's `PvP Rank`, `PvP Wins`, `PvP Losses` and `PvP Matches` attributes; ranked results move a per-season Elo rating (10 placement games at a higher K). Rewards are not decided yet: `ArenaServerEvents.OnMatchEnded` and the template's reward trigger lists are the hooks |
...
| `/spectateteam <n>` | Game masters only, while spectating an arena match. Moves the spectator to team n's spawn. The arena HUD also offers a spectator a **free camera** (right mouse to look, WASD, Q/E) |
