...
1. **Launcher starts** — Fetches the news HTML from `Constants.Configuration.LauncherHtmlUrl` (baked at build time from `GeneratedHostConfig.LauncherHtmlUrl`, which CI substitutes from `FISHMMO_ROOT_DOMAIN`) and extracts the configured `div` class. `IHtmlContentFetcher` yields the **parsed node**, not formatted text — the two launcher views share no text format, and UI Toolkit has no equivalent of TextMeshPro's `<link>` tag, so a news link has to become a real element that can receive a click. Whichever view renders it, every href is routed through `LauncherLinkPolicy`, which parses the URI and allows only absolute `http`/`https` before it reaches `Application.OpenURL`. News is cosmetic: a fetch failure is shown in the news pane and does **not** block the version check.
...
The launcher renders through `ILauncherView`, an interface describing the presentation surface in terms of intent (*show this status*) rather than widget manipulation. `UITKClientLauncher` is the UI Toolkit view; `UGUILauncherView` adapts the original uGUI elements and is used automatically whenever no view component is assigned, so a scene without one is unaffected. All version-check and patch logic stays in `ClientLauncher` and is not forked per view.

**Reverting to uGUI is two fields, no code:** clear **Launcher View Component** on `FishMMOClientLauncher` and re-activate the `Canvas` root. Nothing was deleted — the entire uGUI hierarchy is still in the scene.
