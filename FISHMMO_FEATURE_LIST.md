...
15. **Array Binding** — Batched item writes bind `long[]` and cast `::bigint[]`. They bound `uint[]` and cast `::integer[]`, and **Npgsql 5 cannot bind a CLR `uint[]` at all** — every batched item persist threw at runtime and was recorded as a generic `DATABASE_ERROR`; the `::integer[]` cast would additionally have truncated any stack above `int.MaxValue` had the bind ever succeeded.
...
### Theme and Layering
The whole UI draws from `FishMMO-Theme.uss`: 55 tokens and 137 selectors, with the palette sampled from the project logo art rather than invented — the fish body is `#00364E`, its eye `#0073C0`, the wordmark runs `#0073C0` into `#58B3F1`, and the plate behind it falls to `#001012`. No colour literal appears in any panel stylesheet; every one references a token, so a retheme cannot miss a value hiding in a panel.
