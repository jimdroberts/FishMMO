...
### Launcher
20. **HTML News Feed** — Fetches launcher news via HtmlAgilityPack. `IHtmlContentFetcher` strips `<script>`/`<style>` and yields the **parsed node**; the active view renders it, because the two views share no text format — `HtmlToTmpTextConverter` produces TextMeshPro rich text, `UITKHtmlContentRenderer` builds a `VisualElement` tree (UI Toolkit has no equivalent of TMP's `<link>` tag, so a news link has to be an element that can receive a click). Both bound traversal depth and output size against a hostile document, and every href is opened through `LauncherLinkPolicy`.  
...
27a. **UI Toolkit Launcher View** — `ILauncherView` describes the presentation surface in terms of intent (*show this status*) rather than widget manipulation, so all version-check and patch logic stays in one place instead of being forked per view. `UITKClientLauncher` renders it in UI Toolkit; `UGUILauncherView` adapts the original uGUI elements and is used automatically when no view component is assigned, so any scene without one is unaffected. Reverting is two Inspector fields and no code.  
...
- **`ReleasesCursor` and close-on-escape are one registration.** Showing a panel that releases the cursor also joins the Escape registry, because `PlayerInputController` recaptures the cursor on every frame where nothing is registered as closeable — a panel that released it without registering had it taken back before the player could click anything. Escape is bound to several actions at once, so `UIManager.ClosedThisFrame` stops a handler that would reopen a panel the same press just closed.

**Editor tooling.** `FishMMO → UI Toolkit → Report Unwired Panels` lists every `UITKControl` with no GameObject in the open scene; *Wire Unwired Panels Into Open Scene* creates each one with a `UIDocument`, the shared `PanelSettings`, its UXML and its `Document` field assigned, copies the visibility flags from the legacy panel it replaces, and deactivates that twin. An unwired panel is otherwise indistinguishable from one that was never written — nothing throws, nothing logs, and the only symptom is that a keybind opens the old panel.

### UGUI (Legacy Canvas) Panels
> **Migration status.** `ClientWorldGUI` now runs the UI Toolkit set: 27 panels are wired with a `UIDocument`, the shared `PanelSettings`, their own UXML and their control's `Document` assigned, and each legacy twin of the same name is **deactivated, not deleted**, so rollback stays trivial. `ClientPreboot` carries the UI Toolkit `UITooltip` and `UIDragObject` alongside the legacy pair, which every scene borrows. `UIManager` keeps the two kinds in separate registries and resolves a panel by GameObject name, so a name lookup only ever finds one of them — `TryGet` logs when a UGUI lookup fails for a name that is now registered as a UI Toolkit control, which is otherwise indistinguishable from a panel that was never written.
