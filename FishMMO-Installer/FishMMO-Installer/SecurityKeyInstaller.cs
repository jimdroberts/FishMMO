...
namespace FishMMO.Installer
{
	...
	///
	/// The client build file (ClientApiSecret.generated.cs) is also written so
	/// the gate secret is IL-embedded in the client binary.
	///
	...
	public static class SecurityKeyInstaller
	{
		...
		/// superuser connection. Also writes ClientApiSecret.generated.cs for the
		/// client build.
		///
			...
			bool acceptDefaults = false)
		{
			...
			// ── Always write ClientApiSecret.generated.cs ────────────
			if (!string.IsNullOrWhiteSpace(gateSecret) && Directory.Exists(securityDir))
			{
				string secretPath = Path.Combine(securityDir, "ClientApiSecret.generated.cs");
				var sb = new StringBuilder();
				sb.AppendLine("// AUTO-GENERATED by FishMMO-Installer.");
				...
				sb.AppendLine("\t{");
				sb.AppendLine("\t\tinternal const string Secret = \"" + gateSecret + "\";");
				sb.AppendLine("\t}");
				sb.AppendLine("}");
				await File.WriteAllTextAsync(secretPath, sb.ToString(), Encoding.UTF8);
				await AppSettingsInstaller.ApplySecurePermissions(secretPath);
				Console.WriteLine("  ClientApiSecret.generated.cs written. ✓");
			}
